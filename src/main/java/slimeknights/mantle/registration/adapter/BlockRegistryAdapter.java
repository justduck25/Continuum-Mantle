...
public class BlockRegistryAdapter extends EnumRegistryAdapter<Block> {
  ...
  /** @inheritDoc */
  public BlockRegistryAdapter(BiConsumer<Identifier, Block> register) {
    super(register);
  }

  /** @inheritDoc */
  public BlockRegistryAdapter(BiConsumer<Identifier, Block> register, String modid) {
  ...
  public <T extends Block> T registerOverride(Function<Properties, T> constructor, Block base) {
    return register(constructor.apply(BlockBehaviour.Properties.ofFullCopy(base)), BuiltInRegistries.BLOCK.getKey(base));
  }
  ...
  public WoodBlockObject registerWood(String name, Function<WoodVariant,BlockBehaviour.Properties> behaviorCreator) {
    ...
    WallSignBlock wallSign = register(new MantleWallSignBlock(behaviorCreator.apply(WoodVariant.PLANKS).forceSolidOn().instrument(NoteBlockInstrument.BASS).noCollision().strength(1.0F), woodType), name + "_wall_sign");
    ...
    MantleWallHangingSignBlock wallHangingSign = register(new MantleWallHangingSignBlock(behaviorCreator.apply(WoodVariant.PLANKS).forceSolidOn().instrument(NoteBlockInstrument.BASS).noCollision().strength(1.0F), woodType), name + "_wall_hanging_sign");
