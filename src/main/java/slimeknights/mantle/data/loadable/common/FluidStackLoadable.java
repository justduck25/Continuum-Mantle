...
public class FluidStackLoadable {
  ...
  private static final LoadableField<CompoundTag,FluidStack> NBT = NBTLoadable.ALLOW_STRING.nullableField("nbt", stack -> { CustomData data = stack.get(DataComponents.CUSTOM_DATA); return data == null ? null : data.copyTag(); });
  ...
  private static FluidStack makeStack(Fluid fluid, int amount, @Nullable CompoundTag nbt) {
    ...
    FluidStack stack = holder.areComponentsBound() ? new FluidStack(holder, amount) : new FluidStack(Holder.direct(fluid, DataComponentMap.EMPTY), amount);
    if (nbt != null) {
